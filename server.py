...
class Server:
    ...
    def __init__(self):
        ...
        self.socket.listen(2)  # allow maximum 2 connections to server for game
    ...
    def on_new_client(self, client):
        ...
        client.send(str.encode("Hello " + username + "! Welcome to the game! You can chat in the console."))
        while True:
            msg = client.recv(1024).decode()  # receive a msg
            if not msg:
                break
            else:
                print(msg)  # TODO handle msg
                '''if msg == "STOP":  # client is leaving chat
                print(username + " is leaving the game")  # TODO stop the game
                for c in self.clients:  # inform chat about leaving user  # Code for messenger
